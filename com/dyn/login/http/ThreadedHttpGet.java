...
public class ThreadedHttpGet extends Thread {
	...
	public void run() {
		try {
			...
			sPayload.addProperty("key", ORG_KEY);
			//for some reason this adds backslashes when adding to the LinkedTreeMap...
			token.setParam("payload", sPayload.toString());
			...
			HttpGet getReq = new HttpGet("http://chicago.col-engine-staging.com/partner_organizations/api.json?jwt="+token.serializeAndSign());
			getReq.setHeader("Accept", "application/json");
			getReq.setHeader("Authorization",
					"JWT token=38f5bab69e94db89fac757eed98d900585a05baaa1aa20b71251ca323a53ef92");
